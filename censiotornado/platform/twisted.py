...
from censiotornado.stack_context import NullContext, wrap
from censiotornado.ioloop import IOLoop
from censiotornado.util import timedelta_to_seconds
...
class TornadoReactor(PosixReactorBase):
    ...
    def __init__(self, io_loop=None):
        if not io_loop:
            io_loop = censiotornado.ioloop.IOLoop.current()
...
class _FD(object):
    ...
    def doRead(self):
        if not self.lost:
            self.handler(self.fileobj, censiotornado.ioloop.IOLoop.READ)

    def doWrite(self):
        if not self.lost:
            self.handler(self.fileobj, censiotornado.ioloop.IOLoop.WRITE)

    def connectionLost(self, reason):
        if not self.lost:
            self.handler(self.fileobj, censiotornado.ioloop.IOLoop.ERROR)
            self.lost = True
...
class TwistedIOLoop(censiotornado.ioloop.IOLoop):
