...
from censiotornado.escape import utf8, to_unicode
from censiotornado.test.util import unittest, skipOnAppEngine
from censiotornado.util import unicode_type
...
class TranslationLoaderTest(unittest.TestCase):
    ...
    def clear_locale_cache(self):
        if hasattr(censiotornado.locale.Locale, '_cache'):
            del censiotornado.locale.Locale._cache

    def setUp(self):
        self.saved = {}
        for var in TranslationLoaderTest.SAVE_VARS:
            self.saved[var] = getattr(censiotornado.locale, var)
        self.clear_locale_cache()

    def tearDown(self):
        for k, v in self.saved.items():
            setattr(censiotornado.locale, k, v)
        self.clear_locale_cache()

    def test_csv(self):
        censiotornado.locale.load_translations(
            os.path.join(os.path.dirname(__file__), 'csv_translations'))
        locale = censiotornado.locale.get("fr_FR")
        self.assertTrue(isinstance(locale, censiotornado.locale.CSVLocale))
    ...
    def test_gettext(self):
        censiotornado.locale.load_gettext_translations(
            os.path.join(os.path.dirname(__file__), 'gettext_translations'),
            "tornado_test")
        locale = censiotornado.locale.get("fr_FR")
        self.assertTrue(isinstance(locale, censiotornado.locale.GettextLocale))
        self.assertEqual(locale.translate("school"), u"\u00e9cole")
...
class LocaleDataTest(unittest.TestCase):
    def test_non_ascii_name(self):
        name = censiotornado.locale.LOCALE_NAMES['es_LA']['name']
...
class EnglishTest(unittest.TestCase):
    def test_format_date(self):
        locale = censiotornado.locale.get('en_US')
