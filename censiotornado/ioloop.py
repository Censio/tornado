...
from censiotornado.concurrent import TracebackFuture, is_future
from censiotornado.log import app_log, gen_log
from censiotornado.platform.auto import set_close_exec, Waker
from censiotornado import stack_context
from censiotornado.util import PY3, Configurable, errno_from_exception, timedelta_to_seconds
...
class IOLoop(Configurable):
    ...
    def configurable_default(cls):
        if hasattr(select, "epoll"):
            from censiotornado.platform.epoll import EPollIOLoop
            return EPollIOLoop
        if hasattr(select, "kqueue"):
            # Python 2.6+ on BSD or Mac
            from censiotornado.platform.kqueue import KQueueIOLoop
            return KQueueIOLoop
        from censiotornado.platform.select import SelectIOLoop
        return SelectIOLoop
        ...
        def run():
            try:
                ...
                if result is not None:
                    from censiotornado.gen import convert_yielded
